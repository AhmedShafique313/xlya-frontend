...
// Site's gold brand palette (src/constants/colors.ts) instead of the
// component's original blue/monochrome themes — matte black canvas fill,
// gold grid lines/nodes/ripples.
const THEME = {
  bg: "#000000",
  lineActive: { r: 204, g: 172, b: 93, a: 0.9 }, // --gold-primary #ccac5d
  nodeActive: { r: 204, g: 172, b: 93, a: 1.0 },
  glow: "204,172,93",
  ripple: "227,201,126", // --gold-light #e3c97e
};
...
// Site-wide animated background: a grid that warps toward the cursor and
// ripples on click. Rendered as a fixed, non-interactive layer (like the
// floating-X background it replaced) — drop it once near the top of a
// page's JSX, it never affects document flow.
export default function KineticGrid({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // A named function declaration (not a useCallback) so it can safely
    // reference itself for the rAF loop without a temporal-dead-zone issue.
