...
**Special note — never launch the dev server yourself.** Do not run `npm run dev` (or any other command that starts the Next.js server / opens localhost) on your own initiative. The user runs the frontend themselves. If you need to verify a change works, say so explicitly and ask the user to run it / check localhost, or ask for a screenshot — don't start it in the background and don't assume it's fine because it seems low-risk.
